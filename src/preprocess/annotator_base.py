...
class BaseAnnotator:
    ...
    def __init__(self, preprocessor: Preprocessor, use_cache=True):
        ...
        self.dir_output = self.preprocessor.dir_preprocess / f'annotate.{self.__class__.__name__}'
        self.dir_output.mkdir(exist_ok=True)
        # Instead of a single tokenized file, we now use lists.
        self.tokenized_corpus_files = self.preprocessor.tokenized_corpus_files
        self.tokenized_id_corpus_files = self.preprocessor.tokenized_id_corpus_files
        # The marked corpus will be stored as a list of files (one per partition)
        self.marked_corpus_files = []  # to be set by mark_corpus()
    ...
    def _par_sample_train_data(marked_doc):
        ...
        for sent in sents:
            ...
            all_spans = utils.get_possible_spans(word_idxs, len(sent['ids']),
                                                 consts.MAX_WORD_GRAM, consts.MAX_SUBWORD_GRAM)
    ...
    def sample_train_data(self):
        """
        Process each marked partition separately (without merging).
        Returns a list of file paths for the sampled training data—one per partition.
        """
        sample_files = []
        for marked_path in self.marked_corpus_files:
            path_sampled = self.dir_output / f'sampled.neg{consts.NEGATIVE_RATIO}.{marked_path.name}'
            if self.use_cache and utils.IO.is_valid_file(path_sampled):
                print(f'[SampleTrain] Use cache: {path_sampled}')
            else:
                print(f'[SampleTrain] Processing partition file: {marked_path.name}')
                marked_docs = utils.JsonLine.load(marked_path)
                sampled_docs = [
                    BaseAnnotator._par_sample_train_data(d)
                    for d in tqdm(marked_docs, ncols=100, desc=f'[Sample Train] {marked_path.name}')
                ]
                utils.JsonLine.dump(sampled_docs, path_sampled)
            sample_files.append(path_sampled)
        return sample_files
    ...
    def get_path_sampled_train_spans(path_sampled_train_data):
        ...
        for marked_sent in tqdm(marked_sents, ncols=100, desc='[Annotator] get positive spans'):
            ...
            for l_idx, r_idx in marked_sent['neg_spans']:
                spans.append((0, marked_sent['ids'][l_idx: r_idx + 1]))
        spans = [(label, ''.join(consts.PRETRAINED_TOKENIZER.convert_ids_to_tokens(ids))
                   .replace(consts.GPT_TOKEN, ' ').strip())
    ...
    def mark_corpus(self):
        ...
        partition_marked_files = []
        for tok_path, tok_id_path in zip(self.tokenized_corpus_files, self.tokenized_id_corpus_files):
            marked_path = self.dir_output / f'marked_docs.{tok_path.name}.jsonl'
            if self.use_cache and utils.IO.is_valid_file(marked_path):
                print(f'[Annotate] Use cache: {marked_path}')
            else:
                marked_path = self._mark_corpus_partition(tok_path, tok_id_path)
            # Post-process: Remove empty sentences and docs in this partition.
            docs = utils.JsonLine.load(marked_path)
            for raw_id_doc in docs:
                for sent in raw_id_doc['sents']:
                    sent['phrases'] = [p for p in sent['phrases']
                                       if p[0][1] - p[0][0] + 1 <= consts.MAX_SUBWORD_GRAM]
                raw_id_doc['sents'] = [s for s in raw_id_doc['sents'] if s['phrases']]
            docs = [d for d in docs if d['sents']]
            utils.JsonLine.dump(docs, marked_path)
            partition_marked_files.append(marked_path)
        self.marked_corpus_files = partition_marked_files
        return partition_marked_files
